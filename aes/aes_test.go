...
func TestCFB(t *testing.T) {
	en, err := NewAES256Encrypter(testKey, nil)
	if err != nil {
		t.Errorf("Error:", err)
	}
	en.SetMode(CFB_MODE) // Set CFB_MODE - GCM is default
	ct, err := en.Encrypt(secret)
	if err != nil {
		t.Errorf("Error:", err)
	}
	dc := NewAES256Decrypter(testKey)
	if err != nil {
		t.Errorf("Error:", err)
	}
	dc.SetMode(CFB_MODE) // Set CFB_MODE - GCM is default
